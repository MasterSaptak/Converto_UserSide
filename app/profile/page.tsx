          ...
          <div className="border-2 md:border-4 border-foreground rounded-2xl md:rounded-3xl bg-white p-4 md:p-8 relative overflow-hidden group shadow-[4px_4px_0px_var(--color-foreground)] md:shadow-[6px_6px_0px_var(--color-foreground)] w-full min-w-0">
            ...
            {tierInfo.next ? (
              <div className="text-[10px] md:text-sm uppercase font-black tracking-widest opacity-60 mb-5 md:mb-8">
                {(tierInfo.next.minPoints - (rewards?.lifetime_c_points || 0)).toLocaleString()} C UNTIL {tierInfo.next.name}
              </div>
            ) : (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-5 md:mb-8">
              <div className="bg-secondary/50 border md:border-2 border-foreground/30 md:border-foreground p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-secondary transition-colors min-w-0 md:shadow-[4px_4px_0px_var(--color-foreground)]">
                ...
                <span className="text-sm md:text-xl font-black font-mono truncate block text-primary">{(rewards?.available_c_points || 0).toLocaleString()} C</span>
              </div>
              <div className="bg-secondary/50 border md:border-2 border-foreground/30 md:border-foreground p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-secondary transition-colors min-w-0 md:shadow-[4px_4px_0px_var(--color-foreground)]">
                <span className="text-[9px] md:text-xs uppercase font-black tracking-widest opacity-60 block mb-1 md:mb-1.5">Spent</span>
                <span className="text-sm md:text-xl font-black font-mono truncate block">{(rewards?.spent_c_points || 0).toLocaleString()} C</span>
              </div>
              <div className="bg-secondary/50 border md:border-2 border-foreground/30 md:border-foreground p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-secondary transition-colors min-w-0 md:shadow-[4px_4px_0px_var(--color-foreground)]">
            ...
            <div className="border-t-2 md:border-t-4 border-dashed border-muted pt-4 md:pt-6 min-w-0">
              <div className="text-[10px] md:text-sm font-black uppercase tracking-widest opacity-60 mb-3 md:mb-4">Tier Journey</div>
              
              <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 snap-x relative z-10 custom-scrollbar min-w-0 w-full px-1">
                {REWARD_TIERS.map((tier) => {
                  ...
                  return (
                    <div key={tier.name} className={`flex flex-col items-center justify-center min-w-[90px] md:min-w-[120px] py-3 md:py-4 px-1.5 md:px-2 rounded-xl md:rounded-2xl border-2 md:border-4 snap-center shrink-0 transition-all ${isCurrent ? 'border-foreground bg-secondary scale-105 shadow-[3px_3px_0px_var(--color-foreground)] md:shadow-[6px_6px_0px_var(--color-foreground)] mx-1 md:mx-2' : isPast ? 'border-foreground/30 opacity-70 bg-white' : 'border-foreground/10 bg-secondary/20'}`}>
                      <div className={`w-7 h-7 md:w-10 md:h-10 rounded-full flex items-center justify-center mb-2 md:mb-3 border-2 md:border-4 ${isPast || isCurrent ? 'border-foreground' : 'border-foreground/20'} ${style.bg} ${style.text}`}>
